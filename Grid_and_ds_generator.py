...
for square in squaresQordinations:
    ...
    X_ip = np.zeros(4)
    Y_ip = np.zeros(4)
    UV_in_ip_eleman=[]# 4ta 2 taii rooye ip ha
    ips=[]
    for ip in range(0,4):
        for m in range(0,4):
        ...
        UV_in_ip_eleman.append(V(X_ip[ip], Y_ip[ip]))
    XYips.append(ips)
    UV_elemans.append(UV_in_ip_eleman)
    upwind_coef_4x4=square_upwinder(square,ips,UV_in_ip_eleman)
